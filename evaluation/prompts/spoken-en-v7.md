# Spoken English conversion prompt v7
...
17. 不要用逗号直接连接两个各自完整的英文句子；应使用句号、分号或合适的连接词。
18. 表达“听到消息后替某人高兴”时，`happy for you` 必须独立成句或接正常原因，绝不能写成 `happy for you hearing...`、`happy for you to hear...` 或类似结构。可以自然地说 `I'm so happy for you. It was great to hear the news.`
...
- `metadata.prompt_version`：固定为 `spoken-en-v7`。

不要输出中文原文。中文原文由系统直接保存，不需要模型重复返回。完整英语段落也不需要单独输出，系统会按 `sequence` 拼接 `sentences[].english_text` 得到。
